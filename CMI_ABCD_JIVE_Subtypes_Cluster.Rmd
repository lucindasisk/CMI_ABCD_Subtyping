...
#Compute ranks
(try(cmi_jive_result <- jive(data_1, scale = FALSE, conv=0.05)))
...
```{r run_JIVE_data2, tidy=TRUE, dpi=50}
#Run JIVE analysis for GROUP 2

#Estimate JIVE ranks based on permutation testing (best validated)
#Row-orthogonality enforced between the joint and individual estimates and also between each individual estimate.
#Compute ranks
(try(cmi_jive_result <- jive(data_2, scale = FALSE, conv=0.05)))
...
#Row-orthogonality enforced between the joint and individual estimates and also between each individual estimate.
#Compute ranks
(try(cmi_jive_result <- jive(data_3, scale = FALSE, conv=0.05)))
...
#Row-orthogonality enforced between the joint and individual estimates and also between each individual estimate.
#Compute ranks
(try(cmi_jive_result <- jive(data_4, scale = FALSE, conv=0.05)))
