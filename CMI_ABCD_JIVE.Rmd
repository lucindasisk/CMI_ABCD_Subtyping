...
(cmi_jive_result <- jive(data, scale = FALSE))
(cmi_jive_summary <- summary.jive(cmi_jive_result))
(cmi_jive_var <- showVarExplained(cmi_jive_result, col = c("grey20", "grey43", "grey65")))
(cmi_jive_heatmaps <- showHeatmaps(cmi_jive_result, order_by = 0, show_all = TRUE))
(cmi_jive_pca <- showPCA(cmi_jive_result, n_joint = 0, n_indiv = rep(0, length(result$data)), 
         Colors = "black", pch=1))
