...
myelin <- full_df %>% 
  dplyr::select("subid","uncorrected_group", "corrected_group","zmyelin_set1":"zmyelin_set48")

thick <- full_df %>% 
   dplyr::select("subid","uncorrected_group", "corrected_group","zthickness_set1":"zthickness_set49")

pheno <- full_df %>% 
   dplyr::select("subid","uncorrected_group", "corrected_group","physical_abuse_sum":"fes_p_ss_fc")\

myelin_mat <- myelin %>% 
  dplyr::select(-c("subid","uncorrected_group", "corrected_group")) %>%
  t()
